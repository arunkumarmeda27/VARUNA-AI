...
### Core Requirements
- `numpy`, `scipy`, `pandas`, `pyarrow`, `fastparquet`
...
### What Happens Internally:
1. Trains a calibrated Multi-Class Gradient Boosted Decision Tree (GBDT) on `train_v1.0.0.parquet`.
2. Tunes class weights and regularization parameters on `val_v1.0.0.parquet`.
3. Evaluates on the held-out `test_v1.0.0.parquet` (2024 season), outputting:
   - Overall Accuracy and Macro/Weighted F1 Scores.
   - Multi-Class Brier Score for probability calibration.
   - Confusion Matrix saved to `regimes/evaluation/regime_confusion_matrix.json`.
4. Saves the trained model to `regimes/artifacts/regime_classifier.joblib`.
...
### Output:
- Model binaries saved in `correction/artifacts/`:
  - `level1_eqm_model.joblib`
  - `level2_standard_ml_model.joblib`
  - `level3_regime_aware_model.joblib`
- Performance summary saved in `correction/evaluation/model_ladder_summary.json`.

---

## 6. Step 4: Training Probability & Conformal Uncertainty Quantiles

Calibrates extreme precipitation probabilities and establishes rigorous 80% prediction intervals.

### Executed automatically by verification or via:
```python
...
### Metrics Computed:
1. **Continuous Verification:** Mean Absolute Error (MAE), Root Mean Squared Error (RMSE), Mean Bias, Pearson Correlation Coefficient ($r$).
2. **Categorical / Threat Verification:** Critical Success Index (CSI / Threat Score), Probability of Detection (POD / Hit Rate), False Alarm Ratio (FAR), Equitable Threat Score (ETS) across IMD thresholds:
   - Light Rain ($\ge 2.5\text{ mm}$)
   - Moderate Rain ($\ge 15.6\text{ mm}$)
   - Heavy Rain ($\ge 64.5\text{ mm}$)
   - Very Heavy Rain ($\ge 115.6\text{ mm}$)
   - Extremely Heavy Rain ($\ge 204.5\text{ mm}$)
3. **Probabilistic Verification:** Brier Score, Reliability Curves, Conformal Coverage Rate (empirical coverage $\ge 80\%$).

---

## 8. Automated One-Shot Training Script

To build datasets, train all models, and run verification in a single command, execute:
...
Or run the pre-configured verification runner:
```bash
...
All dataset splits and serialized model binaries are organized systematically:
...
### Q1: How do I change the training/testing year split?
Open `weather_data/master_dataset_builder.py` and modify lines 65–67:
```python
train_df, val_df, test_df = TemporalAligner.create_chronological_splits(
    clean_df,
    time_col="valid_time",
    train_end_year=2022,  # Set training end year
    val_year=2023,        # Set validation year
    test_year=2024,       # Set independent testing year
)
```

### Q2: What if a district has missing telemetry data?
The `DataValidator` in `weather_data/preprocessing/validator.py` automatically flags missing values and uses spatial inverse distance weighting (IDW) interpolation from neighboring stations within a 50 km radius.

### Q3: How do I verify that models are loaded by the web server?
Start the Django server (`python manage.py runserver 127.0.0.1:8000`) and call the health API:
```bash
curl http://127.0.0.1:8000/api/v1/health/
```
Output:
```json
{"status": "healthy", "version": "1.0.0", "database": "connected", "forecast_runs_loaded": 3}
```
